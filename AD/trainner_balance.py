...
from dataloader import read_data,DataLoader,DataLoader_balanced
...
dl_train = DataLoader_balanced(train_data)
dl_test  = DataLoader(test_data)

for i in range(20000): # 10-epochs
    
    x,y,z  = dl_train.sampling(72)
    loss = model.forward(tensor(x).cuda(),tensor(y).cuda(),tensor(z).long().cuda(),True)
    optimizer.zero_grad()
